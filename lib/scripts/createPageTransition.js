...
async function createPageTransition(options) {
    ...
    barba.init({
        ...
        transitions: [{
            ...
            afterEnter({next}){
                ...
                allTargets.forEach(target => target.classList.remove('is-transitioning'))
                allTargets = []
                init()
            },
        }]
    ...
    function animateElement(vars, resolve, thisContainer) {
        ...
        const processAnimation = (options) => {
            ...
            if (options.delay){ //if element has delay, set initial keyframe value
            ...
            targetElement.animate(
                ...
                animationCount++
                if (animationCount === (Array.isArray(vars) ? vars.length : 1)) {
                    return resolve()
                }
            }
        }
