    ...
    function animateElement(vars, resolve, thisContainer) {
        ...
        const processAnimation = (options) => {
            ...
            if (targetElement.length > 1) {
                //if multiple target elements with same selector
                targetElement.forEach((target) => {
                    startAnimation(target, options)
                })
            } else {
                startAnimation(targetElement, options,)
            }
        }
        const startAnimation = (target, options) => {
            ...
            if (options.delay) {
                ...
                Object.values(options.keyframes).forEach((value, index) => {
                    if (Array.isArray(value)) {
                        ...
                        target.style[property] = value[0]
                    }
                })
            }
            target.animate(options.keyframes, {
                ...
                animationCount++
                if (
                    animationCount === (Array.isArray(vars) ? vars.length : 1)
                ) {
                    return resolve()
                }
            }
